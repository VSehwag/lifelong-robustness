# https://github.com/ddkang/advex-uar
import math
import numbers
import numpy as np
import torch
...
def make_kernels(snow_length_bound=13, blur=True):
    ...
    for i in range(7):
        ...
        rr, cc, val = weighted_line(
            mid, mid, np.random.randint(mid + 2, k_size),
            np.random.randint(mid + 2, k_size),
        ...
        kernel = torch.FloatTensor(k_npy.copy()).view(1, 1, k_size,
                                                      k_size).cuda()

        if blur:
            blurriness = np.random.uniform(0.41, 0.6)
            gaussian_blur = GaussianSmoothing(int(np.ceil(5 * blurriness)),
                                              blurriness)
...
def snow_creator(intensities, k, resol):
    ...
    flake_grids = torch.zeros(
        (intensities.size(0), k.size(1), resol, resol)).cuda()
